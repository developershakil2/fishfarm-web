...
const Signup =()=>{
  ...
  const [modalHandle, setModalHandle] = useState('none');
    ...
    const signupHandle = () => {
      ...
      axios.post("http://api.isda.farm/user", formData, {
        headers: {
    ...
    const sendCode = ()=>{
         ...
         axios.post("http://api.isda.farm/sendotp", `+1${phone}`).then((res)=>{
    ...
    axios.post("http://api.isda.farm/getCode",{phone:`+1${phone}`, code:code}).then((res)=>{
